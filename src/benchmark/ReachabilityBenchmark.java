...
public class ReachabilityBenchmark extends Neo4jBenchmark{

    private static final String RESULT_FILE_SUFFIX = "_ap_reachability.csv";
    private static final String CSV_FORMAT = "%.3f,%.3f,%d%n";
    private static final String CSV_HEADER = "prepare_time,run_time,answer";


    public ReachabilityBenchmark(int nodesCount) {
        super(nodesCount);
    }
        ...
        try (PrintWriter outStatsTime = new PrintWriter(new FileOutputStream(outFile, true), true)) {
            ...
            for (int iter = 0; iter < maxIters; ++iter) {
                ...
                if (iter >= warmUpIters && parseResults != null) {
                    outStatsTime.printf(CSV_FORMAT, stepPrepareTime, stepRunTime, parseResults.count());
                }
            }
        }
